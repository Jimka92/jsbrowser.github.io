...
// 2. Скрываем расширение у файлов
function delExtension(inputText) { 
	let regExp = new RegExp(/\.[^.]+$/gi);
	return inputText.replace(regExp, '');  
}

// 3. Разбивка отображения getDate(timestamp) на читаемое отображение времени, даты
...
// 4. Скрываем уведомление об ошибке
...
function createWrapforCanvasComment() {
	...
	// 4. Создание формы для комментариев
function createCommentForm(x, y) {
	...
	// 8. Отправка сообщений сочетанием клавиш Ctrl + Enter
...
function updateCommentForm(newComment) {
	...
	Object.keys(newComment).forEach(id => {
		...
		// 9. Создание формы с сообщениями
...
// Копируем ссылку для клика "поделиться"
